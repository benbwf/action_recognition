...
class TSNDataSet(data.Dataset):
    ...
    def _load_image(self, directory, idx):
        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            ...
            if self.image_tmpl == 'frame_{:010d}.jpg':                
                first_image_number = int(os.path.basename(directory).split('_')[0][2:])
                try:
                    return [Image.open(os.path.join(self.root_path, directory, self.image_tmpl.format(first_image_number+idx-1))).convert('RGB')]
                except Exception:
                    print('error loading image:', os.path.join(self.root_path, directory, self.image_tmpl.format(first_image_number+idx)))
                    return [Image.open(os.path.join(self.root_path, directory, self.image_tmpl.format(first_image_number))).convert('RGB')]
    ...
    def __getitem__(self, index):
        ...
        #print(F"WHAT IS RECORD.PATH = {record.path}") - PATH TO FOLDER CARRYING FRAMES
        ...
        elif self.image_tmpl == 'frame_{:010d}.jpg':
            folder_name = os.path.basename(record.path)
            first_image_number = int(folder_name.split('_')[0][2:])        
            file_name = self.image_tmpl.format(first_image_number)
            full_path = os.path.join(self.root_path, record.path, file_name)
        ...
        #print('################## Found:', os.path.join(self.root_path, record.path, file_name))
        ...
        #print(f'WHAT IS SEGMENT_INDICES: {segment_indices}')
        #print(f'WHAT IS RECORD: {record}')
